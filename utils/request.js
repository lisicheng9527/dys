...
export const myRequest = async (api = '', method = 'POST', params = {}) => {
    ...
    console.log('api=======', api)
    if((config.checkLogin && !wx.getStorageSync('loginToken')) || (config.refreshTokenApi && !wx.getStorageSync('refreshToken'))) {  // 接口请求前检查是否需要登录:1.需要验证登录Api但是没有登录token 2.刷新token接口但是没有刷新token
        await logining();
    ...
    } else if(config.checkLogin && !config.refreshTokenApi && Date.now() - wx.getStorageSync('tokenStartTimeStamp') >= 60 * 60 * 1000) { // 判断token是否过期，token有效期1个小时
        await refreshTokening();
        console.log('refreshTokening------', wx.getStorageSync('loginToken'))
        if(!wx.getStorageSync('loginToken')) {
          wx.showToast({
            title: '网络异常，请稍后重试'
          })
